...
The index file, $LOST_BASE_DIR/data/annotation.idx contains a list of facts on the form

==
fileid(FileId,Filenames,Model,Goal,InputFiles,Options)
==

where Filenames, InputFiles and Options are a lists.

The quadruple (Model,Goal,InputFiles,Options) uniquely identifies FileId and the relation is
...
lost_file_index_get_filenames(IndexFile,Model,Goal,InputFiles,Options,Filenames) :-
	...
	;
	 ...
	 findall(Filename,
			(nth1(FileNo,Filenames,Filename),
			term2atom(FileNo,FileNoAtom),
			atom_concat_list([IndexDir, Model, '_',Goal,'_',IndexAtom,'_', FileNoAtom, '.gen'], Filename)),Filenames),
