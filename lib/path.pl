...
%% clean_file(+Filename,-CleanFileName)
% CleanFileName is Filename but with all double slashes (i.e. //) replaced by a single slash
clean_file(File,CleanFile) :-
	atom_codes(File,FileCodes),
	replace_list("//","/",FileCodes,CleanFileCodes),
	atom_codes(CleanFile,CleanFileCodes).
