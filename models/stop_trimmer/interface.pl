...
lost_option(annotate,optimized,false,'Set a parallel computation of the prediction').
...
% contain stop codons 
stop_trimmer([BlastHspFile],Options,TrimmedHspFile) :-
	open(BlastHspFile,read,InStream),
	open(TrimmedHspFile,write,OutStream),
	stop_trimmmer_rec(InStream,OutStream),
	close(InStream),
	close(OutStream).
